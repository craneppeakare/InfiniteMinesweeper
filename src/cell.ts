...
export default class Cell extends Phaser.GameObjects.Rectangle {
  ...
  private static TILE_POINTS = [0, 10, 80, 270, 640, 1250, 2160, 3430, 5120];
  ...
  constructor(
    ...
    id: { x: number; y: number; chunkId: number },
    chunk: Chunk,
  ) {
    super(scene, pos.x, pos.y, 64, 64, 0xffffff);
    this.setOrigin(0, 0).setInteractive().on("pointerup", this.on_pointerup);

    this.id = id;
    this.chunk = chunk;
    scene.add.existing(this);
    this.label = this.scene.add
      .text(this.getCenter().x, this.getCenter().y, "", Config.defaultStyle)
      .setFontSize(28)
      .setOrigin(0.5, 0.5);
    this.clickHighlight = this.scene.add
      .rectangle(pos.x, pos.y, 64, 64, 0xffffff, 150)
      .setOrigin(0, 0)
      .setVisible(false);
    this.flagImage = this.scene.add
      .image(this.getCenter().x, this.getCenter().y, "flagImage")
      .setVisible(false);
    this.updateColor();
  ...
  reveal(): number {
    ...
    this.updateColor();
    if (this.isAMine) {
      this.mineImage = this.scene.add.image(
        this.getCenter().x,
        this.getCenter().y,
        "mineImage",
      );
      ...
      this.digParticleEmitter.emitParticleAt(
        this.getTopLeft().x,
        this.getTopLeft().y,
      );
      if (this.minesNearby) {
  ...
  private updateColor() {
    ...
    if (this.isRevealed) {
      this.bevelPolygons.forEach((p) => p.destroy());
      this.bevelPolygons = [];
      if (this.isAMine) this.fillColor = isEvenCell ? 0xaa0000 : 0x880000;
      else
        this.fillColor = isEvenCell
          ? Cell.REVEALED_EVEN_COLOR
          : Cell.REVEALED_ODD_COLOR;
    } else {
      this.fillColor = isEvenCell
        ? Cell.COVER_EVEN_COLOR
        : Cell.COVER_ODD_COLOR;
      const x = this.getTopLeft().x;
      const y = this.getTopLeft().y;
      isEvenCell
        ? (this.bevelPolygons = [
            this.scene.add
              .polygon(x, y, "0 0 64 0 60 4 4 4 4 60 0 64", 0x4ec16d)
              .setOrigin(0, 0),
            this.scene.add
              .polygon(x, y, "0 64 4 60 60 60 60 4 64 0 64 64", 0x3a814d)
              .setOrigin(0, 0),
          ])
        : (this.bevelPolygons = [
            this.scene.add
              .polygon(x, y, "0 0 64 0 60 4 4 4 4 60 0 64", 0x41b65b)
              .setOrigin(0, 0),
            this.scene.add
              .polygon(x, y, "0 64 4 60 60 60 60 4 64 0 64 64", 0x2a7a3f)
              .setOrigin(0, 0),
          ]);
    }
  }
